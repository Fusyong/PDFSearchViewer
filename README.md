...
搜索支持正则表达式（含汉字、拼音等中文常用字符集）、跨行、忽略空格等，可按字体、字号、字色等属性过滤。
...
2. **搜索**：左侧输入正则或普通文本，点「搜索」只按文本匹配（含下方规范化选项）。
   ...
   - **批处理**：点「批处理…」（或菜单「文件 → 批处理搜索…」）选择文本文件。编码支持 UTF-8（可带 BOM）、带 BOM 的 UTF-16，以及 GB18030。空行跳过，其余每一行用**当时**的正则、忽略大小写、整词、忽略单个换行、忽略空白各搜一次，全部命中合并为**一次搜索**（聚合、统计、复核、导出、保存会话都与普通搜索相同）。左侧多出「条目筛选」：列出每一条搜索式和命中数，复选框默认全选；取消勾选只是不显示该条的命中。灰色为无命中，红色为表达式无效。样式、坐标、页码过滤同样只影响显示，并会刷新每条后面的条数。若改了搜索选项，点「重新运行」。点「关闭」去掉条目筛选，命中仍全部保留。保存会话时若条目筛选还开着，再次双击该会话会恢复这份清单和勾选状态。
